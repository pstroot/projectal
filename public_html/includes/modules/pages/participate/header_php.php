 ...
 */
 header("location:index.php?main_page=membership");
 exit();
...
if (isset($_GET['action']) && ($_GET['action'] == 'send')) {
	$error = false;
	if($isParticipant){	
		$messageStack->add('join', "You are already a participant");
		//$error = true;
	}
	...
	if (!$error) {
		...
		if (empty($firstname)) {
		  $messageStack->add('join', FIRST_NAME_BLANK_ERROR);
		  $error = true;
		}else if (strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
		  $messageStack->add('join', FIRST_NAME_TOO_SHORT_ERROR);
		  $error = true;
		}
		
		
		if (empty($lastname)) {
		  $messageStack->add('join', LAST_NAME_BLANK_ERROR);
		  $error = true;
		}else if (strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
		  $messageStack->add('join', LAST_NAME_TOO_SHORT_ERROR);
		  $error = true;
		}
		
		if (empty($birthday)) {
		  $messageStack->add('join', AGE_BLANK_ERROR);
		  $error = true;
		} 
		...
		if (strlen($email) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
		  ...
		  $messageStack->add('join', CONFIRMATION_EMAIL_BLANK_ERROR);
		  $error = true;
		} else if (strtolower(trim($email)) != strtolower(trim($email_confirmation))) {
		  $messageStack->add('join', CONFIRMATION_EMAIL_ERROR);
		  $error = true;
		...
		/*
		if (empty($city)) {
		  $messageStack->add('join', CITY_BLANK_ERROR);
		  $error = true;
		}
		
		if (empty($state)) {
		  $messageStack->add('join', STATE_BLANK_ERROR);
		  $error = true;
		}
		
		if (empty($zip)) {
		  $messageStack->add('join', ZIP_BLANK_ERROR);
		  $error = true;
		}
	
		if (empty($phone)) {
		  $messageStack->add('join', PHONE_BLANK_ERROR);
		  $error = true;
		}
		*/
		...
		if (!isset($_POST["loggedInUserID"])) { // if the user is not logged in, then we are  creating a new account, so validate the password and email
			if (empty($password)) {
			  ...
			  $messageStack->add('join', PASSWORD_TOO_SHORT_ERROR);
			  $error = true;
...
if (!$error) {
	...
	if (!isset($_POST["loggedInUserID"])) { 
		...
		$sql = "INSERT INTO " . TABLE_CUSTOMERS . " 
										(customers_firstname,customers_lastname,customers_telephone, customers_password, customers_email_address,customers_newsletter,customers_email_format)
										VALUES (
										'" . $firstname . "', 
										'" . $lastname . "', 
										'" . $phone . "', 
		...
		if(!$error){
			...
			if(!$db->Execute($sql)){	
				  $messageStack->add('join', 'There was an error updating the user information<BR><BR>$sql');
				  $error = true;
			}
		} 
		...
		if(!$error){
			...
			if ($verbose) print $sql . "<BR><BR>";
			if(!$db->Execute($sql)){	
				  $messageStack->add('join', 'There was an error updating the address<BR><BR>$sql');
				  $error = true;
			}
		}
		if(!$error){
			...
			if ($verbose) print $sql . "<BR><BR>";
			if(!$db->Execute($sql)){	
				  $messageStack->add('join', 'There was an error removing existing mailinglist selections<BR><BR>$sql');
				  $error = true;
			}
		}
	}
...
if(!$error && !$isParticipant){
	$sql = "INSERT INTO participants
			(customer_id,birthday,profession,date_created)
			VALUES 
			(" . (int)$_SESSION['customer_id'] . ",
									'$birthday', 
									'$profession', 
	...
	if ($verbose) print $sql . "<BR><BR>";
	if(!$db->Execute($sql)){	
		$messageStack->add('join', 'There was an error adding the customer to the participants database<BR><BR>$sql');
		$error = true;
	}
	...
	if(!$error){
	    zen_redirect(zen_href_link(FILENAME_PARTICIPATE, 'action=success'));
	}
