...
if (isset($_POST['action']) && (($_POST['action'] == 'process') || ($_POST['action'] == 'update'))) {
  ...
  if ($error == false) {
    ...
    if ($_POST['action'] == 'update') {
	  ...
	  // If this is there is not a default address set yet for this user, set this new one as the default address -pstroot
	  $default_address_query = "SELECT customers_default_address_id FROM " . TABLE_CUSTOMERS . " WHERE customers_id = :customersID AND customers_default_address_id > 0";
	  $default_address_query = $db->bindVars($default_address_query, ':customersID', $_SESSION['customer_id'], 'integer');
	  $daq = $db->Execute($default_address_query);
      if ($daq->RecordCount() <= 0) {
		$sql_data_array = array(array('fieldName'=>'customers_default_address_id', 'value'=>$new_address_book_id, 'type'=>'integer'));
		$where_clause = "customers_id = " . $_SESSION['customer_id'];
    ...
    projectal_zen_redirect(zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'));
  }
}
...
if($entry->fields['entry_country_id'] == '' || $entry->fields['entry_country_id'] == 0)  $entry->fields['entry_country_id'] = 223;// added by pstroot to solve the problem of no states being populated before a country was selected
