...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  ...
  if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
    ...
    $featured_products_query = "select distinct p.products_id, p.products_image, pd.products_name, p.master_categories_id, p.master_categories_id
                                from (" . TABLE_PRODUCTS . " p
                                left join " . TABLE_FEATURED . " f on p.products_id = f.products_id
                                left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id)
                                where p.products_id = f.products_id
...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
	...
	$theListingID = $featured_products->fields['products_id'];
	$theProductName = $featured_products->fields['products_name'];
	$theListingHref = zen_href_link(zen_get_info_page($theListingID), 'cPath=' . $productsInCategory[$theListingID] . '&products_id=' . $theListingID);
	
	$theProductName = projectal_get_listing_title($theProductName,$theListingID,$theListingHref);
	
    if (!isset($productsInCategory[$theListingID])) $productsInCategory[$theListingID] = zen_get_generated_category_path_rev($featured_products->fields['master_categories_id']);
    $list_box_contents[$row][$col] = array('params' =>'class="centerBoxContentsFeatured centeredContent back productHoverBox"' . ' ' . 'style="width:' . ($col_width-.05) . '%;"',
	
	// zen_image to projectal_zen_image by PAUL STROOT
	// projectal_zen_image($src, $alt = '', $width = '', $height = '', $parameters = '', $productID = '', $categoryID) 
      'text' => (($featured_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . $theListingHref . '">' . 
	  
	  projectal_zen_image(
	  	DIR_WS_IMAGES . $featured_products->fields['products_image'], 
		...
		$featured_products->fields['master_categories_id']
	  ) . '</a><br />') . '<a href="' . $theListingHref . '">' . $theProductName . '</a><br />' . $products_price);
