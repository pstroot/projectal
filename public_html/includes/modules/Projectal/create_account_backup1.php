...
if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  if (strlen($email_address) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
	...
	// check to see if this email address already exists, but NOT a user account
    $check_account_query = "SELECT COUNT(*) as total, customers_id
                            FROM " . TABLE_CUSTOMERS . "
                            WHERE customers_email_address = '" . zen_db_input($email_address) . "' 
							AND customers_password = ''";
    $check_account = $db->Execute($check_account_query);
    if ($check_account->fields['total'] > 0) {
      	$updateExistingAccount = $check_account->fields['customers_id'];
		$messageStack->add('create_account', "Your email already exists, so we've updated your account with your new information");
    }
	  
	 if(!isset($updateExistingAccount)){
  ...
  // CHECK FOR DUPLICATE NICKNAMES IN ZEN CART OR PHPBB
...
/*
// updated by PAUL STROOT - commented out to remove country validation from form submission
...
*/
  ...
  if ($error == true) {
    ...
    $zco_notifier->notify('NOTIFY_FAILURE_DURING_CREATE_ACCOUNT');
	  ...
	  // if we're updating an existing account, as determined above (line 136), then do an update instead of an insert.
		if(isset($updateExistingAccount)){
			$action = "update";
			$_SESSION['customer_id'] = $updateExistingAccount;
			$paremeters = 'customers_id = ' . $updateExistingAccount;
		} else {
			$action = "insert";
			$paremeters = '';
		}
    ...
    zen_db_perform(TABLE_CUSTOMERS, $sql_data_array,$action,$paremeters);
