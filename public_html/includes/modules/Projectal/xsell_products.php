...
if (isset($_GET['products_id']) && SHOW_PRODUCT_INFO_COLUMNS_XSELL_PRODUCTS > 0 ) {
  $xsell_query = $db->Execute("select distinct cd.categories_name, p.products_id, p.products_image, pd.products_name
                                 from " . TABLE_PRODUCTS_XSELL . " xp, " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " pc, " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd
...
while (!$xsell_query->EOF) {
  ...
  if(strtoupper($xsell_query->fields['categories_name']) == "MEN") $xsell_query->fields['categories_name'] = "Guys";
  if(strtoupper($xsell_query->fields['categories_name']) == "WOMEN") $xsell_query->fields['categories_name'] = "Ladies";
  ...
  'id' => $xsell_query->fields['products_id'],
  'name' => $xsell_query->fields['products_name'],
  'category' => $xsell_query->fields['categories_name'],
  'image' => $xsell_query->fields['products_image']
  );
  $col ++;
