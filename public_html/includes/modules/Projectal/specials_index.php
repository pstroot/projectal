...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  ...
  if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
    ...
    $specials_index_query = "select distinct p.products_id, p.products_image, pd.products_name, p.master_categories_id, p.master_categories_id
                             from (" . TABLE_PRODUCTS . " p
                             left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id
                             left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id )
                             where p.products_id = s.products_id
...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="centerBoxContentsSpecials centeredContent back productHoverBox"' . ' ' . 'style="width:' . ($col_width-.1) . '%;"',
    // zen_image to projectal_zen_image by PAUL STROOT
	// projectal_zen_image($src, $alt = '', $width = '', $height = '', $parameters = '', $productID = '', $categoryID) 
	'text' => (($specials_index->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'cPath=' . $productsInCategory[$specials_index->fields['products_id']] . '&products_id=' . (int)$specials_index->fields['products_id']) . '">' . 
	projectal_zen_image(
	  	DIR_WS_IMAGES . $specials_index->fields['products_image'], 
		...
		$specials_index->fields['master_categories_id']
	 ) . '</a><br />') . '<a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'cPath=' . $productsInCategory[$specials_index->fields['products_id']] . '&products_id=' . $specials_index->fields['products_id']) . '">' . $specials_index->fields['products_name'] . '</a><br />' . $products_price);
