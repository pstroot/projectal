...
//                        $selected_attribute = ($products_options->fields['attributes_default']=='1' ? true : false);
                    ...
                    switch ($products_options_names->fields['products_options_images_style']) {
					  ...
					  // CASE 6 added by Paul Stroot
					  case '6':  // ROUNDED CORNER BOX
						  $tmp_radio .= "<li style='display:none;'>" . zen_draw_radio_field('id[' . $products_options_names->fields['products_options_id'] . ']', $products_options_value_id, $selected_attribute, 'id="' . 'attrib-' . $products_options_names->fields['products_options_id'] . '-' . $products_options_value_id . '"') . "</li>";
						  $tmp_radio .= "<li><a href=\"javascript:changeSize('".$products_options->fields['products_options_values_id']."')\" id=\"size_".$products_options->fields['products_options_values_id']."\">".$products_options->fields['products_options_values_name']."</a></li>";
						  //$tmp_radio .= '<label class="attribsRadioButton one" for="' . 'attrib-' . $products_options_names->fields['products_options_id'] . '-' . $products_options_value_id . '">' . ($products_options->fields['attributes_image'] != '' ? zen_image(DIR_WS_IMAGES . $products_options->fields['attributes_image'], '', '', '' ) . '  ' : '') . $products_options_details . '</label><br />' . "\n";
                      break;
                    }
                  }
				...
				// THIS BLOCK ADDED BY PAUL STROOT
				  if ($products_options->fields['attributes_default'] == 1){					
					$default_option_id = $products_options->fields['options_values_id'];
					$options_javascript[] = "<SCRIPT> setDefaultAttribute(".$products_options_names->fields['products_options_id'].",".$products_options->fields['options_values_id']."); </SCRIPT>\n"; // ADDED BY PAUL STROOT
				  }
				  
				  // THIS BLOCK ADDED BY PAUL STROOT
				  if($products_options_names->fields['products_options_id'] == 1){ // "1" is the ID for the "SIZE" attributes. Show the size chart button if it exists
					  $options_javascript_2[] = "<SCRIPT> showSizingChart(); </SCRIPT>\n"; 
				  }
                ...
                switch (true) {
                  ...
                  case ($products_options->RecordCount() == 1): 
					  if ($show_attributes_qty_prices_icon == 'true') {
						...
						//$options_name[] = ""; // ADDED BY PAUL STROOT
						$options_name[] = $products_options_names->fields['products_options_name'];
					  }
					  
					  $options_menu[] = zen_draw_hidden_field('id[' . $products_options_names->fields['products_options_id'] . ']', $products_options_value_id, 'id="' . 'attrib-' . $products_options_names->fields['products_options_id'] . '-' . $products_options_value_id . '"', $selected_attribute) . "\n";// UPDATED BY PAUL STROOT
					  $options_comment[] = $products_options_names->fields['products_options_comment'];
					  $options_comment_position[] = ($products_options_names->fields['products_options_comment_position'] == '1' ? '1' : '0');
					  break;
