...
//echo 'I SEE ' . (int)$cPath_array[$i] . '<br>';
    if ($categories->RecordCount() > 0) {
		$theName = $categories->fields['categories_name'];
		$theLink = zen_href_link(FILENAME_DEFAULT, 'cPath=' . implode('_', array_slice($cPath_array, 0, ($i+1))));
		
		
		if($i == 0){ // if this is the first item after the "HOME" link.
			$theName = "SHOP";
			$theLink = "index.php?main_page=store";
		}
      $breadcrumb->add($theName, $theLink);
