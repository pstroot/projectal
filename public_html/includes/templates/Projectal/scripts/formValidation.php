...
$errorTemplate = "<BR><font color='#CC0000'><b>* ::ERROR::</b></font>";
...
// untaintData
function cleanUp($array, $index, $maxlength){
	if (isset($array["{$index}"])){
		if (count($array["{$index}"]) <= 1){ 						// if this is NOT an array
			$maxlength = 10000;
			...
			foreach ($array["{$index}"] as $val){
				$thisVar = trim(substr($val,0,$maxlength));
				$thisVar = addslashes($thisVar);
...
function bytesToMegabytes($b,$decimals=1){

	$mb = $b/(1024/1024);

    return number_format($mb,$decimals);
