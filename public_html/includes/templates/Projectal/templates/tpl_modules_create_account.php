...
?>
<div class="formRadioBlock" style="margin-top:6px;">
	<?php echo zen_draw_checkbox_field('newsletter', '1', $newsletter, 'id="newsletter-checkbox"')  . " " . ENTRY_NEWSLETTER  . (zen_not_null(ENTRY_NEWSLETTER_TEXT) ? '<span class="alert">' . ENTRY_NEWSLETTER_TEXT . '</span>': ''); ?>
    <br class="clearBoth" />
    <?php } ?>
</div>
...
while (!$newsletterCustom->EOF) { ?>
	<div class="formRadioBlock">
        ...
        <?php echo zen_draw_checkbox_field('custom_mailinglist_' . $newsletterCustom->fields['list_id'], $newsletterCustom->fields['list_id'], false, 'id="newsletter"'); ?>
        <? echo $newsletterCustom->fields['list_name']; ?>
        <span class="note">(<? echo $newsletterCustom->fields['list_description']; ?>)</span>
        <br class="clearBoth" />
    </div>
	<? 
