...
if (isset($_GET["setActiveState"])){
	$Query = "UPDATE charities SET isActive = {$_GET["setActiveState"]} WHERE charity_id = {$_GET["id"]}";
...
<head>
	...
	function doAdd(){	
		...
		var url = "charities_edit.php"
		var title = "ADD CHARITY"
		startLytebox(url,title,rev) 
	}
	function doEdit(id){	
		var rev = "width: 600px; height: 700px; scrolling: auto;border:3px solid #CC0000;"
		var url = "charities_edit.php?id=" + id
		var title = "EDIT CHARITY"
		startLytebox(url,title,rev) 
	}
	
	function doDelete(id){
		var rev = "width: 500px; height: 200px; scrolling: auto;border:3px solid #CC0000;"
		var url = "charities_delete.php?id=" + id
		var title = "DELETE CHARITY"
		startLytebox(url,title,rev) 
	}
...
<table id="theDataTable">
	<tr>
		...
		createColumnHeader('Name','name');
		createColumnHeader('Description','description');
		?>   
       <!-- <th></th> -->
    </tr>
    ...
    <?PHP
		...
		$Query = "SELECT * FROM charities ORDER BY $sortby $sortDir";
		$Results = $db->Execute($Query);
		 while (!$Results->EOF) {	
			  $charity_id = $Results->fields['charity_id'];
			  $name = $Results->fields['name'];
			  $description = $Results->fields['description'];
			  $isActive =  $Results->fields['isActive'];
			  ?>
			  <tr onclick="" id="row_<?=$charity_id; ?>">
				<td align="center"><div id="active_<?=$charity_id; ?>"><input type="checkbox" value="<?=$charity_id;?>" <? if($isActive == 1) print "checked"; ?> onChange="changeActiveState(this)"></div></td>
                <td><div id="name_<?=$charity_id; ?>"><?= $name; ?></div></td>
                <td><div id="address_<?=$charity_id; ?>"><?= stripslashes($description); ?></div></td>
				<!--
