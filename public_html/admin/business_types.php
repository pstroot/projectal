...
$sortby = "type_name"; // default sort field 
...
if (isset($_GET["setActiveState"])){
	$Query = "UPDATE business_types SET isActive = {$_GET["setActiveState"]} WHERE type_id = {$_GET["id"]}";
...
<head>
    ...
    <title>Admin - Business Types</title>  	
	...
	function doAdd(){	
		...
		var url = "business_types_edit.php"
		var title = "ADD NEW BUSINESS TYPE"
		startLytebox(url,title,rev) 
	}
	function doEdit(id){	
		var rev = "width: 500px; height: 200px; scrolling: auto;border:3px solid #CC0000;"
		var url = "business_types_edit.php?id=" + id
		var title = "EDIT BUSINESS TYPE"
		startLytebox(url,title,rev) 
	}
	
	function doDelete(id){
		var rev = "width: 500px; height: 200px; scrolling: auto;border:3px solid #CC0000;"
		var url = "business_types_delete.php?id=" + id
		var title = "DELETE BUSINESS TYPE"
		startLytebox(url,title,rev) 
	}
...
<input type="button" value="Add New Business Type" style="float:right;margin-left:5px;" class="blackBtn" onclick="doAdd()" />
...
<table id="theDataTable">
	<tr>
		...
		createColumnHeader('Name','type_name');
		?>   
        <th></th>
    </tr>
    
    <?PHP
		$Query = "SELECT * FROM business_types ORDER BY $sortby $sortDir";
		$Results = $db->Execute($Query);
		 while (!$Results->EOF) {	
