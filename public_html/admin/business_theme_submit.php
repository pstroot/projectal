...
if (strtoupper($_POST["submit"]) == "ADD"){	
	$theOrder = 0;
	$Query = "SELECT theOrder FROM themes ORDER BY theOrder DESC LIMIT 1";
	...
	$Query = "INSERT INTO themes (theme_name,theOrder)
							VALUES
								('{$_SESSION["formVars"]["theme_name"]}',
								  $theOrder);";
...
if (strtoupper($_POST["submit"]) == "EDIT"){
	$Query = "UPDATE themes SET 
								theme_name = '{$_SESSION["formVars"]["theme_name"]}'
							WHERE theme_id = {$_SESSION["formVars"]["id"]};";
