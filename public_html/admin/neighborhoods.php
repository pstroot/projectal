...
$sortby = "neighborhood_name"; // default sort field 
...
if (isset($_GET["setActiveState"])){
	$Query = "UPDATE neighborhoods SET isActive = {$_GET["setActiveState"]} WHERE neighborhood_id = {$_GET["id"]}";
...
<head>
    ...
    <title>Admin - Neighborhoods</title>  	
	...
	function doAdd(){	
		...
		var url = "neighborhood_edit.php"
		var title = "ADD NEIGHBORHOOD"
		startLytebox(url,title,rev) 
	}
	function doEdit(id){	
		var rev = "width: 500px; height: 200px; scrolling: auto;border:3px solid #CC0000;"
		var url = "neighborhood_edit.php?id=" + id
		var title = "EDIT NEIGHBORHOOD"
		startLytebox(url,title,rev) 
	}
	
	function doDelete(id){
		var rev = "width: 500px; height: 200px; scrolling: auto;border:3px solid #CC0000;"
		var url = "neighborhood_delete.php?id=" + id
		var title = "DELETE NEIGHBORHOOD"
		startLytebox(url,title,rev) 
	}
...
<input type="button" value="Add New Neighborhood" style="float:right;margin-left:5px;" class="blackBtn" onclick="doAdd()" />
<h1>Neighborhoods</h1>
...
<table id="theDataTable">
	<tr>
		...
		createColumnHeader('Name','neighborhood_name');
		?>   
        <th></th>
    </tr>
    
    <?PHP
		$Query = "SELECT * FROM neighborhoods ORDER BY $sortby $sortDir";
		$Results = $db->Execute($Query);
		 while (!$Results->EOF) {	
			  $id = $Results->fields['neighborhood_id'];
			  $name = $Results->fields['neighborhood_name'];
