	...
	class ImageMagick_noUpload {
		...
		var $verbose        = false;
			...
			*/
			...
			function ImageMagick_noUpload($dir,$filename) {				
				
				$this->temp_file = $filename;
				if(!@rename($dir . $filename, $this->temp_dir.'/tmp'.$this->count.'_'.$this->temp_file)){
					$errors["image"] = "Imagemagick: Upload failed";
					if($this->verbose == TRUE) {
						echo "Imagemagick: Upload failed";
					}
				}
