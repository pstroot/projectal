...
if (isset($_GET["setActiveState"])){
	$Query = "UPDATE faq_categories SET isActive = {$_GET["setActiveState"]} WHERE cat_id = {$_GET["id"]}";
...
<head>
    ...
    <title>Admin - F.A.Q. Categories</title>  	
	...
	function doAdd(){	
		...
		var url = "faq_categories_edit.php"
		var title = "ADD NEW F.A.Q. CATEGORY"
		startLytebox(url,title,rev) 
	}
	function doEdit(id){	
		var rev = "width: 500px; height: 200px; scrolling: auto;border:3px solid #CC0000;"
		var url = "faq_categories_edit.php?id=" + id
		var title = "EDIT F.A.Q. CATEGORY"
		startLytebox(url,title,rev) 
	}
	
	function doDelete(id){
		var rev = "width: 500px; height: 200px; scrolling: auto;border:3px solid #CC0000;"
		var url = "faq_categories_delete.php?id=" + id
		var title = "DELETE F.A.Q. CATEGORY"
		startLytebox(url,title,rev) 
	}
...
<input type="button" value="Add New Category" style="float:right;margin-left:5px;" class="blackBtn" onclick="doAdd()" />
<h1>F.A.Q. Categories</h1>
...
<table id="theDataTable">
    ...
    <?PHP
		...
		$Results = $db->Execute($Query);
		 while (!$Results->EOF) {	
			  $id = $Results->fields['cat_id'];
			  $name = $Results->fields['cat_name'];
