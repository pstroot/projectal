...
if(isset($_GET["id"]) && !isset($_SESSION["bounceFromErrors"])){
	...
	while (!$Results->EOF) {	
		  $_SESSION["formVars"]["theme_name"] = $Results->fields['theme_name'];
		  $Results->MoveNext();
	}
}

if(isset($_GET["id"])) {
	$pageTitle = "Edit \"" . @$_SESSION["formVars"]["theme_name"] . "\"";
	$buttonLabel = "Edit";
	$successURL = "business_theme_updated.php";
} else {
	$pageTitle = "Add New Business Theme";
	$buttonLabel = "Add";
	$successURL = "business_theme_added.php";
}
?>
...
<form action="business_theme_submit.php" method="post" enctype="multipart/form-data">
<div class="formTable">
<table>
	<tr>
        <th>Name</th>        
        <td colspan="3"><input type="text" name="theme_name" id="theme_name" value="<?=@$_SESSION["formVars"]["theme_name"];?>" /><?=required("theme_name");?></td>
    </tr>
