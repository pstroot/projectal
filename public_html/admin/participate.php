...
$sortby = "p.date_created"; // default sort field 
...
if (isset($_GET["setActiveState"])){
	$Query = "UPDATE participants SET isActive = {$_GET["setActiveState"]} WHERE customer_id = {$_GET["id"]}";
	print $Query;	
...
<head>
    ...
    <script>
	function doAdd(){	
		var rev = "width: 600px; height: 400px; scrolling: auto;border:3px solid #CC0000;"
		var url = "participate_add.php"
		var title = "ADD PARTICIPANT"
		startLytebox(url,title,rev) 
	}
	function doEdit(id){	
		var rev = "width: 600px; height: 400px; scrolling: auto;border:3px solid #CC0000;"
		var url = "participate_edit.php?id=" + id
		var title = "EDIT PARTICIPANT"
		startLytebox(url,title,rev) 
	}
	
	function doDelete(id){
		var rev = "width: 500px; height: 300px; scrolling: auto;border:3px solid #CC0000;"
		var url = "participate_delete.php?id=" + id
		var title = "DELETE PARTICIPANT"
		startLytebox(url,title,rev) 
	}
	
	</script>
</head>
<body onLoad="activateNav('nav5')">
<a id="lytebox_misc"></a>
<? include("includes/inc_header.php"); ?>

<input type="button" value="Add New Participant" style="float:right;margin-left:5px;" class="blackBtn" onclick="doAdd()" />
...
<table id="theDataTable">
	<tr>
		...
		createColumnHeader('First Name','customers_firstname');
		createColumnHeader('Last Name','customers_lastname');
		createColumnHeader('Since','date_created');
		createColumnHeader('Email','customers_email');
		createColumnHeader('Profession','profession');
		?>   
        <th></th>
    </tr>
    
    <?PHP
		$Query = "SELECT  p.profession, p.participant_id, c.customers_firstname, c.customers_lastname, p.date_created, c.customers_email_address, p.isActive FROM participants p
		...
		$Results = $db->Execute($Query);
		 while (!$Results->EOF) {	
			  ...
			  <tr id="row_<?=$id; ?>">
                ...
                <td><div id="lastname_<?=$id;?>"><?= $lastname; ?></div></td>
                <td><?= $since; ?></td>
                <td><div id="email_<?=$id;?>"><?= $email; ?></div></td>
                <td><div id="profession_<?=$id;?>"><?= $profession; ?></div></td>
