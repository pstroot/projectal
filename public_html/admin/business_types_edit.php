...
if(isset($_GET["id"]) && !isset($_SESSION["bounceFromErrors"])){
	...
	while (!$Results->EOF) {	
		  $_SESSION["formVars"]["type_name"] = $Results->fields['type_name'];
		  $Results->MoveNext();
	}
}

if(isset($_GET["id"])) {
	$pageTitle = "Edit \"" . @$_SESSION["formVars"]["type_name"] . "\"";
	$buttonLabel = "Edit";
	$successURL = "business_types_updated.php";
} else {
	$pageTitle = "Add New Business Type";
	$buttonLabel = "Add";
	$successURL = "business_types_added.php";
}
?>
...
<form action="business_types_submit.php" method="post" enctype="multipart/form-data">
<div class="formTable">
<table>
	<tr>
        <th>Name</th>        
        <td colspan="3"><input type="text" name="type_name" id="type_name" value="<?=@$_SESSION["formVars"]["type_name"];?>" /><?=required("type_name");?></td>
    </tr>
