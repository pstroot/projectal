...
//]]>
                ...
                function addContent($content = '') {
                    ...
                    if ($this->_options['like_topleft']=='yes'||$this->_options['tweet_topleft']=='yes'||$this->_options['stumble_topleft']=='yes' || $this->_options['googleplus_topleft']=='yes'){
                        ...
                        $buildspace .= $this->interactive_holder($this->doGooglePlusButton($postid), 'left');
                        }
                 ...
                 if ($this->_options['like_topright']=='yes'||$this->_options['tweet_topright']=='yes'||$this->_options['stumble_topright']=='yes' || $this->_options['googleplus_topright']=='yes'){
                       ...
                       if($this->_options['googleplus_topright']=='yes'){
                        $buildspace .= $this->interactive_holder($this->doGooglePlusButton($postid), 'right');
                        }
                       $content = $buildspace."</div>".$content;
                    }
               ...
               protected function get_twitter_suffix($postid, $tweet){
                ...
                function interactive_func($atts, $content) {
                        ...
                        if($tweet=='yes'){
                        $buildup.= $this->doTweetiFrame($postid, $perma, '', $title, $tweet_look, $tweet_size, $faces);
                        }
                        if($like=="yes"){
                        $buildup.= $this->doLikeiFrame($postid, $perma, $like_look,$facebook_size);
                        }
                        if($stumble=='yes'){
                ...
                //
                protected function replaceKeyWordsInURL($share_url,$page_link, $page_title, $page_excerpt ){
                ...
                //
               protected function replaceKeyWordsInPopup ($page_id, $page_title, $popup_text){
                ...
                // set base attributes
                 ...
                 switch ($special){
                     ...
                     case "twitter":
                         ...
                         break;
                     case "clean":
                         if ($css_images=="yes"){$result = "<a rel=\"".$rel."\" target=\"".$target."\"  ".$this->doImageStyle($css_class, $size, $list_style)." href=\"".$this->replaceKeyWordsInURL($sites[$css_class]['share_url'], $page_link, $page_title, $page_excerpt )."\" title=\"".$this->replaceKeyWordsInPopup ($page_id, $page_title, $popup_text)."\"><span class=\"head\">".stripslashes  ($share_text)."</span></a>";}
                         else{$result.="<a rel=\"".$rel."\" target=\"".$target."\" href=\"".$this->replaceKeyWordsInURL($sites[$css_class]['share_url'], $page_link, $page_title, $page_excerpt )."\" title=\"".$this->replaceKeyWordsInPopup ($page_id, $page_title, $popup_text)."\" ><img src=\"".WP_PLUGIN_URL."/share-and-follow/images/blank.gif\" style=\"background: transparent url(".$this->getIconSprites( $size).") no-repeat;padding:0;margin:0;height:".$size."px;width:".$size."px;background-position:".str_replace(" ", "px ",$sites[$css_class]['sprites'][$size])."px\" class=\"image-".$size."\" height=\"".$size."\"  width=\"".$size."\"  alt=\"".$image_name."\"/> <span class=\"head\">".stripslashes ($share_text)."</span></a>";}
                         break;
                     case "bookmark":
                     ...
                     case "rss":
                        ...
                        if ($css_images=='yes'){$result ="<a rel=\"".$target."\" ".$this->doImageStyle($css_class, $size, $list_style)." title=\"".$this->replaceKeyWordsInPopup ($page_id, $page_title, $popup_text)."\"><span class=\"head\">".stripslashes  ($share_text)."</span></a>";}
              ...
              function getCDNurlStamped($file, $time = '30' ){
               ...
               function getIconSprites( $size ){
                    ...
                    if($this->_options['cdn']['status_txt']!='OK'){
                        ...
                        $file = CDNDIRECTORY.$this->_options['icon_set']."/".$size."/sprite-".$size.".png";
                        $directory = $this->getCDNurlStamped($file);
                    }
                return $directory;
                }
                ...
                if ((strlen($this->_options['cdn-key']) == 40)&&!empty($this->_options['cdn'])){
                 ...
                 if($this->_options['cdn']['status_txt']=="FAIL" || $result === false ){
                     ...
                     $result = $control->__toString();
                         if($result != false){
                             set_transient('cndrep', $result, 60*60*24*14);
                         }
        ...
        public function social_links($args =  array() ){
                ...
                $args = wp_parse_args( $args, $this->_defaults  );
                extract( $args, EXTR_SKIP );
    ...
    if($stumble=='yes'){
    $html.= $this->interactive_holder($this->doStumbleScript($postid, $perma, $stumble_look, $size,$stumble_size ), 'show_interactive');
    }
    if($googleplus=='yes'){
        $html.= $this->interactive_holder($this->doGooglePlusButton($postid, $perma, $google_style, $google_size), 'show_interactive');
    }
echo $html;
}
...
echo $output;
        ...
        // end admin pages
             ...
             function check_for_gooogle(){
                 if( $this->_options['googleplus_topleft'] == 'yes' || $this->_options['googleplus_topright'] == 'yes' || $this->_options['googleplus_bottom'] == 'yes' ){
                     $this->google_plus_footer_code();
                 }
             }
...
//Actions
	...
	add_action('wp_head', array(&$cons_shareFollow, 'getCDNcodes'),1); // adds items into head section
        ...
        add_action('wp_footer',array(&$cons_shareFollow, 'check_for_gooogle'),100); // adds google plus code when needed
